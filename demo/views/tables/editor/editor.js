 ...
 export default class Editor extends Component {
     componentDidMount() {
         ...
         this.editor.config.uploadImgUrl = config.uploadImgUrl;
         this.editor.config.uploadParams = config.uploadParams;
         this.editor.config.uploadHeaders = config.uploadHeaders;
         ///* 菜单配置，默认为全部菜单
         this.editor.config.menus = ((!!config.menus) && config.menus.length > 0) ? config.menus : defaultconfig.menus;
         if (config.lang) {
             this.editor.config.lang = wangEditor.langs[config.lang];
         }
         this.editor.config.hideLinkImg = true;
         this.editor.config.emotions = config.emotions;
         this.editor.create();
         ...
         // 禁用
         (this.props.disabled) && this.editor.disable();

         // 启用
         (!this.props.disabled) && this.editor.enable();
     }
     clearContent() {
     ...
     render() {
         ...
         const hasToolbox = this.props.hasToolbox || false;
         return <div className="editor-box">
                   {(!!hasToolbox)&&<div className="tool-box">
                        <button onClick={this.clearContent.bind(this)}>清空</button>
