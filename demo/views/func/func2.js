...
export default class Func2 extends React.Component {
  ...
  clone(){
    ...
    this.calendar.fullCalendar('renderEvent', {
      title, start, end, allDay,color,id:nextId }, true);
    
    start && (start = start.format('YYYY-MM-DD HH:mm'));
    end && (end = end.format('YYYY-MM-DD HH:mm'));

    console.log({
      action,title,allDay,color, start, end,
      id:nextId
    });//此处数据用来通知后台修改数据 --- 新增数据
  ...
  handleOk() { //点击弹框内的OK
    let { action, start, end, title, color, allDay, id,
      isEnd, _allDay, _end, _id, _start, nextId} = this.state;
    if (title == ''){
    ...
    switch(action){
      ...
      case 'edit':
        this.calendar.fullCalendar('updateEvent', {start,_allDay,_end,_id,_start,
          end, title, color, allDay, id});
  ...
  componentDidMount() {
    ...
    this.calendar = $('#calendar').fullCalendar({
      ...
      eventDrop: function(event) {
        ...
        console.log({
          action:'edit',title,allDay,color,id,start,end
        });//此处数据用来通知后台修改数据
      ...
      select: function(start, end) {
        that.setState({
          start, end, action:'add',isEnd:true
        })
        that.showModal();
      },
      ...
      eventClick: function(calEvent, jsEvent, view) { //点击某一个事件
        ...
        const isEnd = !!end;
        that.setState({ action: 'edit',
          _allDay, _end, _id,_start, start, end, title, color, allDay, id, isEnd });
        that.showModal();
      },
      events: json
    })
  ...
  render() {
    ...
    switch(this.state.action){
      case 'add' : calEvent = '新建事件';footer = [btn[2],btn[3]]; break;
      case 'edit': calEvent = '编辑事件';footer = btn; break;
      default    : calEvent = '编辑事件';footer = btn; break;
    }
    return (
      <div>
